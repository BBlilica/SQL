--형변환 함수
--자동형변환으 제공합니다. number <-> 문자, DATE <-> 문자
SELECT * FROM EMPLOYEES WHERE SALARY >= '10000';                 //   <ㅜ 자동 형변환
SELECT HIRE_DATE FROM EMPLOYEES WHERE HIRE_DATE = '05/09/30';// </

-- 강제형변환
--TO_CHAR = 날짜 -> 문자로 강제 형변환 (날짜 포맷형식 쓰입니다.)
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY-MM-DD HH:MI:SS') FROM DUAL;
SELECT SYSDATE, TO_CHAR(SYSDATE, 'YYYY"년" MM"월" DD"일"') FROM DUAL;
SELECT FIRST_NAME, TO_CHAR(HIRE_DATE, 'YY-MM-AM') FROM EMPLOYEES;

-- TO_CHAR = 숫자 -> 문자로 강제 형변환 (숫자 포맷형식 쓰입니다)
SELECT TO_CHAR(20000, '999999999') FROM DUAL; -- 9는 자리수
SELECT TO_CHAR(20000, '0999999999') FROM DUAL; -- 남는 자리수 0으로 채움
...
SELECT FIRST_NAME, SALARY, COMMISSION_PCT, SALARY + SALARY * NVL(COMMSSION_PCT, 0) AS 실제급여 FROM EMPLOYEES;

-- NVL2 (타겟, 'NOT NULL이 아닐 때, 'NULL일 때') 
...
SELECT FIRST_NAME
    , JOB_ID
    , SALARY
    , CASE JOB_ID WHEN 'IT_PORG' THEN SALARY * 1.1
...
-- COALESCE(값, 값, ....) 코알레스 - NULL이 아닌 첫번째 인자값을 변환하는 함수
SELECT COALESCE('A', 'B', 'C') FROM DUAL;
SELECT COALESCE(NULL, 'B','C') FROM DUAL;
SELECT COALESCE(NULL, NULL, 'C') FROM DUAL;
--SELECT COALESCE(COMMISSON_PCT, 0), NVL(COMMISSION_PCT 

------------------------------------------------------------------------------------------------------------------

--문제 1.
--1) 오늘의 환율이 1302.69원 입니다 SALARY컬럼을 한국돈으로 변경해서 소수점 2자리수까지 출력 하세요.
SELECT CONCAT(SALARY *1302.69, '원') AS 한국돈 FROM EMPLOYEES;
SELECT TO_CHAR(SALARY * 1302.69, 'L999,999.00') FROM EMPLOYEES;
...
SELECT HIRE_DATE, FIRST_NAME || ' ' ||LAST_NAME FROM EMPLOYEES ORDER BY HIRE_DATE - SYSDATE DESC;
